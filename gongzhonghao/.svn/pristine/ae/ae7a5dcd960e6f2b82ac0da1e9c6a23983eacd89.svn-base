...
//通过config接口注入权限验证配置
wx.config({
    debug: false,//alert {"errMsg":"config:ok"}
...
wx.ready(function () {
    ...
    wx.checkJsApi({
        ...
        success: function(res) {
            ...
            var html = '<p>checkJsApi chooseWXPay '+res.checkResult.chooseWXPay+'</p>';
            $('#info2').append(html);
        }
    });
            ...
            wx.chooseImage({
                ...
                success: function (res) {
                    ...
                    $.each(localIds,function(index, item){
                        html += '<a href="javascript:;" class="weui-grid"><div class="weui-grid__icon"><img src="'+item+'" alt="" width="100px" height="100px"></div></a>';
                    });
                    $('#cimgs').append(html);
                    $('#cimgs img').on('click', function(){
                            //预览图片接口
                            wx.previewImage({
                            current: $(this).attr('src'), // 当前显示图片的http链接 或 weixin://resourceid/3693c0602c075d6ed305efd3204e2947
        ...
        $('#c3').on('click',function(){
            ...
            if(imgs.length > 0){
                $.each(imgs, function(k,v){
                    //上传图接口 一次一张  上传图片有效期3天，可用微信多媒体接口下载图片到自己的服务器，此处获得的 serverId 即 media_id。
                    wx.uploadImage({
                        localId: $(v).attr('src'),  // 需要上传的图片的本地ID，由chooseImage接口获得
                        isShowProgressTips: 1, // 默认为1，显示进度提示
                        success: function (res) {
                            var serverId = res.serverId; // 返回图片的服务器端ID
                            $(v).attr('serverId', serverId);
                            downloadimg.push(serverId);
                            //$('#info2').append('<p>uploadImage '+serverId+'</p>');
                            //alert(downloadimg.toString());
                        },
                        fail: function (res) {
                ...
                $.each(downloadimg, function(k,v){
                    ...
                    wx.downloadImage({
                        ...
                        success: function (res) {
                            ...
                            html += '<a href="javascript:;" class="weui-grid"><div class="weui-grid__icon"><img src="'+localId+'" alt="" width="100px" height="100px"></div></a>';
                        },   
                        fail: function (res) {
                            alert(JSON.stringify(res));
                        }
