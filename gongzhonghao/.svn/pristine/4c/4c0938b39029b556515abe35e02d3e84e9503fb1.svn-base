...
if($_GET['type'] == 3){
    ...
    $json = json_encode(array('articles' => array($media)));
    //$r = $news->addNews(array('body' => $json));
    $r = $news->addNews(array('articles' => array($media)));
    if(empty($r) || $r["errcode"] > 0){
        echo "错误代码：".$r["errcode"]." ".$r["errmsg"];
    }else{
        var_dump($r);
...
//获取素材 
if($_GET['type'] == 4){
    //$media = '{ "type":news, "offset":1, "count":20 }';
    if($_GET['types'] == 1){
    $media = array(
        "type" => 'image',
        "offset" => 1, 
    ...
    $r = $news->batchget_material($media); 
    }
    if($_GET['types'] == 2){
    $media =$media = '{ "type":image, "offset":1, "count":20 }';
    $r = $news->batchget_material($media); 
    }
    ...
    $r = $news->batchget_material(array('body' => $media)); 
    }if($_GET['types'] == 4){
    $media = array(
        "type" => 'image',
        "offset" => 1, 
    ...
    $r = $news->batchget_material(array('body' => json_encode($media))); 
    }
    if(empty($r) || $r["errcode"] > 0){
        echo "错误代码：".$r["errcode"]." ".$r["errmsg"];
    }else{
